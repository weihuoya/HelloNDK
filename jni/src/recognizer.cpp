...
{
    logWrite("new recognizer: %d", type);
}
...
uint32_t TapRecognizer::process(const Input * input)
{
    ...
    logWrite("tap: %d, %f, %d", size, input->distance, input->deltaTime, input->type);
    ...
    if(size == pointers_ && input->distance < moveThreshold_ && input->deltaTime < taptime_)
    {
        if(input->type == Input::INPUT_START && tapCount_ == 0)
        {
            ...
            timer_ = Timer::instance()->setTimeout(callback, interval_);

            logWrite("tap start failed timer");
        }
        else if(input->type == Input::INPUT_END)
        {
            ...
            if(validInterval && distance < offsetThreshold_)
            {
                ...
                logWrite("tap start trigger timer");
            }
            else
            {
                tapCount_ = 0;
...
uint32_t PinchRecognizer::process(const Input * input)
{
    ...
    logWrite("pinch finger: %d, scale: %f", size, input->scale);
...
uint32_t RotateRecognizer::process(const Input * input)
{
    ...
    logWrite("rotate finger: %d, rotation: %f", size, input->rotation);

    if(size == pointers_)
    {
        float rotation = abs(input->rotation);
...
uint32_t SwipeRecognizer::process(const Input * input)
{
    ...
    logWrite("swipe finger: %d, velocity: %f (%f,%f)", size, input->velocity, input->velocityX, input->velocityY);
